  ...
  methods: {
      ...
      defaulcode(){
         var params  = new URLSearchParams();
          params.append('type', this.$route.query.ttype);
          params.append('num', this.$route.query.tnum);
          this.$http.post("/api/count/settypeAndnum",params, {
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded'
            }
          })
...
<style lang="scss" scoped>
  ...
  .code_bg{background: #fff;border-radius: 6px;padding: 15px; padding-bottom:60px;position: relative}
  .title_head{ border-bottom:1px solid #f5bf66}
  .yellow{color: #f5bf66}
  .small_head{ position: absolute;top:51%;left:45%;width: 30px; z-index: 999;}
</style>
